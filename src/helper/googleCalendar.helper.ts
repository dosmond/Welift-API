...
export class GoogleCalendarApiHelper {
  ...
  constructor() {
    this.google = JSON.parse(fs.readFileSync(path.resolve(__dirname, "../keys/google-service.json")))
  }
  ...
  private async getAccessToken(): Promise<string> {
    ...
    let { data } = await axios.post("https://oauth2.googleapis.com/token", {
  ...
  createGoogleCalendarEvent = async ({ state, description, title, start, end }) => {
    let calendarId = this.getCalendarId(state)
  ...
  updateGoogleCalendarEvent = async ({ state, description, title, start, end, eventId }) => {
    let calendarId = this.getCalendarId(state)
    return axios.put(`https://www.googleapis.com/calendar/v3/calendars/${calendarId}/events/${eventId}`,
      {
        start: {
          dateTime: start
        },
  ...
  deleteGoogleCalendarEvent = async ({ state, eventId }) => {
    let calendarId = this.getCalendarId(state)
    return axios.delete(`https://www.googleapis.com/calendar/v3/calendars/${calendarId}/events/${eventId}`,
      {
        headers: {
          Authorization: `Bearer ${await this.getAccessToken()}`
        }
